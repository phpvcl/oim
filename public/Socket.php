...
class Socket
{
    ...
    const EXPIRES = 300;
    //api地址
    const API_URL = 'http://www.oim.com/chat/';
    ...
    public function onTask($server, $task_id, $src_worker_id, $data)
    {
        ...
        switch ($data['emit'])
        {
            ...
            case 'status'://修改在线状态
                ...
                $this->notifyOnOff($server, $data['key'], $data['status']);
                break;
            case 'offlineMessage':
    ...
    public function saveMessage($server, $fromKey, $toKey, $data)
    {
        ...
        if (!$message = $server->redis->hGet(self::MESSAGE_QUEUE, $chatKey))
        {
            ...
            $message = $server->redis->hGet(self::MESSAGE_QUEUE, $chatKey);
        }
     ...
     * @return void
     */
    public function notifyOnOff($server, $key, $status = 'online')
    {
        if ($server->connections)
        {
            $data = ['emit' => 'status', 'id' => intval($key), 'status' => $status];
            ...
            if ($status == 'offline')
            {
                ...
                if (is_string($value))
                {
                    ...
                    foreach ($users as $toKey)
                    {
                        ...
                        if($message && $this->callFunction('ChatRecord/save', $data))
                        {
